...
class AuthDatasource {
  final baseUrl = 'http://194.31.53.102:21082';
  final String apiKey = 'ZXZhZHdpMzEzQGdtYWlsLmNvbTpldmExMjM=';
  ...
  Future<EditProfileResponseModel> editProfile({
    ...
    final token = await AuthLocalStorage().getToken();
    var headers = {'Authorization': 'Bearer $token', 'x-api-key': apiKey};
  ...
  Future<ForgotPasswordResponseModel> forgotPassword(String email) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Gagal mengirim email reset kata sandi: ${responseBody['message']}');
      }
    } catch (e) {
  ...
  Future<void> validateToken(String token) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Token tidak valid atau kadaluarsa: ${responseBody['message']}');
    }
  }
